...
int main(int argc, char* argv[]) {
    ...
    const optimisation::loss loss{ optimisation::losses::trivial() };
    for (int i = 0; i < 10; ++i) {
        const double error_squared = 0.5 * static_cast<double>(i);
        math::matrix<double, 3, 1> rho;
        loss.compute(error_squared, rho);
        REQUIRE(rho[0] == error_squared);
        REQUIRE(rho[1] == 1.0);
        REQUIRE(rho[2] == 0.0);
        REQUIRE(is_value_approx(numerical_derivative(loss, error_squared + 1.0), 1.0, 1e-6));
    }
