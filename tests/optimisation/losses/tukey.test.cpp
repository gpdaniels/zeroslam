...
int main(int argc, char* argv[]) {
    ...
    const optimisation::loss loss{ optimisation::losses::tukey(delta) };

    for (int i = 0; i < 9; ++i) {
        const double error_squared = (delta * delta) * 0.1 * static_cast<double>(i);
        math::matrix<double, 3, 1> rho;
        loss.compute(error_squared, rho);
        REQUIRE(rho[0] >= 0.0);
        REQUIRE(rho[0] <= (delta * delta) / 3.0);
        REQUIRE(is_value_approx(rho[1], numerical_derivative(loss, error_squared + 1e-3), 1e-4));
        REQUIRE(rho[2] <= 0.0);
    }
