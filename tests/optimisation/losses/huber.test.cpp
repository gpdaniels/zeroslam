...
int main(int argc, char* argv[]) {
    ...
    const optimisation::loss loss{ optimisation::losses::huber(delta) };

    {
        math::matrix<double, 3, 1> rho;
        loss.compute(1.0, rho);
        REQUIRE(rho[0] == 1.0);
        REQUIRE(rho[1] == 1.0);
        REQUIRE(rho[2] == 0.0);
    }
    ...
    {
        math::matrix<double, 3, 1> at;
        loss.compute(delta * delta, at);
        math::matrix<double, 3, 1> above;
        loss.compute((delta * delta) * (1.0 + 1e-9), above);
        REQUIRE(is_value_approx(at[0], above[0], 1e-6));
        for (int i = 1; i < 10; ++i) {
            const double error_squared = (delta * delta) * (1.0 + static_cast<double>(i));
            math::matrix<double, 3, 1> rho;
            loss.compute(error_squared, rho);
            REQUIRE(is_value_approx(rho[0], (2.0 * std::sqrt(error_squared) * delta) - (delta * delta)));
