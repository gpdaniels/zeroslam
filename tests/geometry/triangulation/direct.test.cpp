...
int main(int argc, char* argv[]) {
    ...
    // Exact recovery.
    {
        ...
        for (int trial = 0; trial < 64; ++trial) {
            ...
            for (int sample = 0; sample < 16; ++sample) {
                ...
                REQUIRE(geometry::triangulation::direct<double>::triangulate(normalise(lhs_camera), lhs_pose, normalise(rhs_camera), rhs_pose, result));
                ...
                REQUIRE(geometry::triangulation::direct<double>::triangulate(lhs_camera, lhs_pose, rhs_camera * 0.5, rhs_pose, result_from_rays));
    ...
    // Noisy observations stay close.
    {
        ...
        for (int trial = 0; trial < 64; ++trial) {
            ...
            for (int sample = 0; sample < 16; ++sample) {
                ...
                math::matrix<double, 3, 1> result;
                REQUIRE(geometry::triangulation::direct<double>::triangulate(lhs_point, lhs_pose, rhs_point, rhs_pose, result));
    ...
    // Parallel rays and a known point.
    {
        ...
        const math::matrix<double, 3, 1> ray = { { 0.1, 0.0, 1.0 } };
        math::matrix<double, 3, 1> result;
        REQUIRE(!geometry::triangulation::direct<double>::triangulate(ray, lhs_pose, ray, rhs_pose, result));
        REQUIRE(!geometry::triangulation::direct<double>::triangulate(math::matrix<double, 3, 1>::zero(), lhs_pose, ray, rhs_pose, result));
        REQUIRE(!geometry::triangulation::direct<double>::triangulate(ray, lhs_pose, ray, lhs_pose, result));

        const math::matrix<double, 3, 1> lhs_ray = { { 1.0, 0.5, 4.0 } };
        const math::matrix<double, 3, 1> rhs_ray = { { -1.0, 0.8, 4.5 } };
        REQUIRE(geometry::triangulation::direct<double>::triangulate(lhs_ray, lhs_pose, rhs_ray, rhs_pose, result));
        ...
        for (int trial = 0; trial < 64; ++trial) {
            ...
            for (int sample = 0; sample < 16; ++sample) {
                ...
                if ((sample % 4) == 0) {
                    ...
                    math::matrix<double, 3, 1> result;
                    REQUIRE(geometry::triangulation::direct<double>::triangulate(lhs_camera * random.get_random(0.5, 2.0), lhs_pose, rhs_camera * random.get_random(-2.0, -0.5), rhs_pose, result));
    ...
    // Single precision.
    {
        ...
        for (int trial = 0; trial < 16; ++trial) {
            ...
            for (int sample = 0; sample < 8; ++sample) {
                ...
                REQUIRE(geometry::triangulation::direct<float>::triangulate(lhs_point, lhs_pose_float, rhs_point, rhs_pose_float, result));
