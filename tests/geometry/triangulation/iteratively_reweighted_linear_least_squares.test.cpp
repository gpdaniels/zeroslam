...
int main(int argc, char* argv[]) {
    ...
    // Exact recovery.
    {
        ...
        for (int trial = 0; trial < 64; ++trial) {
            ...
            for (int sample = 0; sample < 16; ++sample) {
                ...
                REQUIRE(geometry::triangulation::iteratively_reweighted_linear_least_squares<double>::triangulate(normalise(lhs_camera), lhs_pose, normalise(rhs_camera), rhs_pose, result));
                ...
                REQUIRE(geometry::triangulation::iteratively_reweighted_linear_least_squares<double>::triangulate(lhs_camera, lhs_pose, rhs_camera * 0.5, rhs_pose, result_from_rays));
    ...
    // Noisy observations stay close.
    {
        ...
        for (int trial = 0; trial < 64; ++trial) {
            ...
            for (int sample = 0; sample < 16; ++sample) {
                ...
                math::matrix<double, 3, 1> result;
                REQUIRE(geometry::triangulation::iteratively_reweighted_linear_least_squares<double>::triangulate(lhs_point, lhs_pose, rhs_point, rhs_pose, result));
    ...
    // Degenerate rays and depth signs.
    {
        ...
        math::matrix<double, 3, 1> result;
        REQUIRE(!geometry::triangulation::iteratively_reweighted_linear_least_squares<double>::triangulate(zero_ray, lhs_pose, normal_ray, rhs_pose, result));
        REQUIRE(!geometry::triangulation::iteratively_reweighted_linear_least_squares<double>::triangulate(normal_ray, lhs_pose, zero_ray, rhs_pose, result));

        const math::matrix<double, 3, 1> lhs_ray = { { 1.0, 0.5, 4.0 } };
        const math::matrix<double, 3, 1> rhs_ray = { { -1.0, 0.8, 4.5 } };
        REQUIRE(geometry::triangulation::iteratively_reweighted_linear_least_squares<double>::triangulate(lhs_ray, lhs_pose, rhs_ray, rhs_pose, result));
        ...
        const math::matrix<double, 3, 1> world = { { 0.2, 0.1, 2.0 } };
        const math::matrix<double, 3, 1> rhs_ray_behind = { { world[0], world[1], world[2] - 5.0 } };
        REQUIRE(geometry::triangulation::iteratively_reweighted_linear_least_squares<double>::triangulate(world, lhs_pose, rhs_ray_behind, rhs_pose_behind, result));
        REQUIRE(is_value_approx(result[0], world[0]));
        REQUIRE(is_value_approx(result[1], world[1]));
        REQUIRE(is_value_approx(result[2], world[2]));
        REQUIRE(transform(lhs_pose, result)[2] > 0.0);
        REQUIRE(transform(rhs_pose_behind, result)[2] < 0.0);
    }
        ...
        for (int trial = 0; trial < 64; ++trial) {
            ...
            for (int sample = 0; sample < 16; ++sample) {
                ...
                if ((sample % 4) == 0) {
                    ...
                    math::matrix<double, 3, 1> result;
                    REQUIRE(geometry::triangulation::iteratively_reweighted_linear_least_squares<double>::triangulate(lhs_camera * random.get_random(0.5, 2.0), lhs_pose, rhs_camera * random.get_random(-2.0, -0.5), rhs_pose, result));
    ...
    // Single precision.
    {
        ...
        for (int trial = 0; trial < 16; ++trial) {
            ...
            for (int sample = 0; sample < 8; ++sample) {
                ...
                REQUIRE(geometry::triangulation::iteratively_reweighted_linear_least_squares<float>::triangulate(lhs_point, lhs_pose_float, rhs_point, rhs_pose_float, result));
