...
# Find all source headers, test headers, tool headers, and public headers.
GET_PROJECT_FILES(GUARDED_FILES
    "include/*.h"
    "include/*.hpp"
    "source/*.hpp"
    "tests/*.hpp"
...
FOREACH(GUARDED_FILE ${GUARDED_FILES})
    ...
    # Determine the include guard name from the file path:
    # - Generally the project prefix `ZEROSLAM_` followed by the path below the top level directory.
    # - The library headers and the public include headers drop the source or include/zeroslam part.
    SET(GUARDED_FILE_NAME "${GUARDED_FILE}")
    STRING(REGEX REPLACE "^(source|include/zeroslam)/" "" GUARDED_FILE_NAME "${GUARDED_FILE_NAME}")
    STRING(REGEX REPLACE "[./-]" "_" GUARDED_FILE_NAME "${GUARDED_FILE_NAME}")
    STRING(TOUPPER "ZEROSLAM_${GUARDED_FILE_NAME}" GUARDED_FILE_NAME)

    # Prepare the guard regex.
    SET(GUARD_REGEX_LINES
        "^#pragma once$"
        "^#ifndef ${GUARDED_FILE_NAME}$"
        "^#define ${GUARDED_FILE_NAME}$"
        "^#endif // ${GUARDED_FILE_NAME}$"
    )
    ...
    LIST(FIND GUARD_DATABASE "${GUARDED_FILE_NAME}" INDEX_DATABASE)
    ...
    ENDIF()
    LIST(APPEND GUARD_DATABASE "${GUARDED_FILE_NAME}")
